...
func NewFileSourceClient(clientId, clientSecret, tokenFile string, googleClientPath string, authFn authCodeFn) (*http.Client, error) {
	...
	if !exists || token.RefreshToken == "" {
		...
		googleClient.ClientId = clientId
		googleClient.ClientSecret = clientSecret
	
		SaveGoogleClient(googleClientPath,&googleClient)
	}
