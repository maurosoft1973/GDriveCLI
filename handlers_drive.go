...
const GoogleFileCredentials = "GOOGLE_CREDENTIALS.json"
const DefaultCacheFileName = "file_cache.json"

type GoogleClientStruct struct {
...
func getOauthClient(args cli.Arguments) (*http.Client, error) {
	...
	googleClientPath := ConfigFilePath(configDir, GoogleFileCredentials)

	jsonFile, err := os.Open(googleClientPath)
	if err != nil {
		fmt.Println(err)
		clientId = args.String("clientId")
		clientSecret = args.String("clientSecret")
	} else {
		// read our opened jsonFile as a byte array.
		byteValue, _ := ioutil.ReadAll(jsonFile)

		var googleClient GoogleClientStruct

		// we unmarshal our byteArray which contains our
		json.Unmarshal(byteValue, &googleClient)
	...
	return auth.NewFileSourceClient(clientId, clientSecret, tokenPath, googleClientPath, authCodePrompt)
}
